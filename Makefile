...
deploy: deploy-0 deploy-1 deploy-2 deploy-b

undeploy: undeploy-0 undeploy-1 undeploy-2 undeploy-b
...
deploy-b: $(BACKEND) frontend/build/index.html
	scp -r frontend/build/ pi@$(HOST2):/tmp/www
	ssh pi@$(HOST2) sudo mkdir -p /var/lib/e-curtain
	ssh pi@$(HOST2) sudo rm -rf /var/lib/e-curtain/www
	ssh pi@$(HOST2) sudo mv /tmp/www /var/lib/e-curtain/
	script/deploy.sh $(BACKEND) $(HOST2) /var/log/e-curtain.bin $(HOST0)
